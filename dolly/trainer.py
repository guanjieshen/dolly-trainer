...
from .dbfs_sync import LocalToOutputDirSync
...
def train(
    ...
    local_to_output_dir_sync = (
        LocalToOutputDirSync(local_dir=local_output_dir, output_dir=dbfs_output_dir) if dbfs_output_dir else None
    )

    if not local_to_output_dir_sync:
    ...
    trainer = Trainer(
        ...
        callbacks=[local_to_output_dir_sync] if local_to_output_dir_sync else [],
    )
